...
| `visibility` | `public` = بيانات تجريبية · `restricted` = بيانات فعلية. **من 2026-09-19 اللوحة مقفولة بالكامل: الاثنين لا يقرأهم إلا مشرف أو عضو في القطاع؛ الزائر ما يقرأ شي.** |
...
### `backupPlans/{sectorId}` — خطط التغذية البديلة
نفس منطق نموذج الإكسل المعتمد عند فريق التشغيل: عنصر رئيسي وله بدائل (لين 3 أو أكثر)، والحساب بالأمبير.

`sectorId`, `visibility: 'restricted'` (القواعد ترفض غيرها)، `ratingA` (سعة القاطع الافتراضية، 400)، `updatedAt`, `cases`:

```
{ id, level: 'station' | 'feeder', voltageKv, ratingA?,        // ratingA يتجاوز الافتراضي لهذي الحالة
  main:    { no, loadA },
  backups: [{ no, loadA }, ...], note? }
```

الحساب (في الفرونت، دالة نقية): السعة المتاحة لكل بديل = السعة − حمله؛ القابل للاستعادة = الأقل بين حمل الرئيسي ومجموع المتاح؛ التوزيع على البدائل متناسب مع المتاح. مواقع المحطات على الخريطة تجي من دليل المحطات في `mapLayerIndex` بمطابقة الرقم — ما تتخزن هنا.

مستند واحد لكل قطاع (قراءة واحدة). لو تجاوز عدد الحالات بضعة آلاف ينقسم لأجزاء بنفس أسلوب `networkBundles`.
